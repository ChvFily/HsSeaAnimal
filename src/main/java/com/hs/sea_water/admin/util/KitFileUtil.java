package com.hs.sea_water.admin.util;
...
//            return getError("上传目录不存在。");
        ...
        String localPath[] = new String[imgFile.length] ;
...
//                }
                ...
                try{
                    ...
                    FileCopyUtils.copy(imgFile[i].getInputStream(), new FileOutputStream(savePath));

                }catch(Exception e){
                    return getError("上传文件失败。");
                }
                url[i] = saveUrl + newFileName;
                localPath[i] = savePath;
            }
        }
        map.put("error", 0);
        map.put("url", url);
